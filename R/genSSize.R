...
#' @description The Shiny app displays a chart of effect size vs. sample size from which estimated sample sizes can be read off for multiple tests.
...
#' Shiny side panel explains the sample size calculator and asks for information about the design (one or two group) and the approximate degrees of freedom (DOF) for t-tests.
    ui <- fluidPage(sidebarLayout(
      conditionalPanel(
        ...
        div(style = "display: flex; flex-wrap: wrap;color:black",
          ...
          "Study",
          label = h6(
            "Enter 0 for a single group design; enter the proportion of the larger group size for a two group design.",
            width = "150px"
          ),
          value = 0.5,
          min = 0,
        ...
        numericInput(
          "DOF",
          label = h6(
            "Nominate the approximate degrees of freedom of the t-tests. You can refine this later.",
            width = "150px"
          ),
          value = 38,
          min = 4,
      ...
      mainPanel(
        width = 9,
        titlePanel(h3(
          "A Visual Tool to Help Design Useful Statistical Tests"
        )),
        div(
          ...
          conditionalPanel(
            ...
            with a standard 2-sided null hypothesis test. Selecting the funnel plot option will take you to a generalisation of funnel plots.
            "
          )
        ),
        conditionalPanel(
   ...
   #' Then the estimated effect size and variance are requested (or study means and vaiances if drawing a generalized funnel plot).
        ...
        div(
          ...
          column(width = 2, div(
            ...
            numericInput(
              "xmax",
              label = h6(em("range of effect sizes"), width = "100px"),
              value = 1.5,
              min = .2,
              step = .1,
              width = "100px"
   ...
   div(style = "display: flex; flex-wrap: wrap;color:black",
       column(8, p(h5(
          em(
            "The calculations used here are explained in 'A visual tool for the design of statistical analyses' by Aisbett, Drinkwater, Quarrie & Woodcock.
          ...
          datadisplay(input$Study,
            ...
            output$clickss <-
              renderText({
                xhit = 0
                yhit = 0.01
                xhit <- input$plot_click$x
                yhit <- input$plot_click$y
                clickreturn<-clickPoints(input$FunnelPlot, input$Study, var, xhit, yhit) #  convert to sample size if required
                paste(
                  "Click chart to get effect size and",
                  clickreturn[1],
                  "at a point: (",
                  clickreturn[2],
                  ",",
...
#' @references  'A visual tool for the design of statistical analyses' by Aisbett, Drinkwater, Quarrie & Woodcock.
#' @export
